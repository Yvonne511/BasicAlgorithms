	...
	public static void DTH(int n, int from_rod, int to_rod, int other_rod) {
		if (n==1) {
			if (IsItThere(n, from_rod)) {
				...
				initial[other_rod/10-1][0] = 0;
				initial[to_rod/10-1][0] = 1;
			}
		}
		else {
			//find out where is the largest
			if (IsItThere(n, from_rod)) {
				DTH(n - 1, to_rod, other_rod, from_rod);
				System.out.println("Move disk "+ n + " from rod " + from_rod +" to rod " + to_rod );
				initial[from_rod/10-1][n-1] = 0;
				initial[to_rod/10-1][n-1] = n;
				DTH(n - 1, other_rod, to_rod, from_rod);
				//move the rest to C
				//move n from A to B
				//move the rest to B
			}
				...
				DTH(n - 1, to_rod, from_rod, other_rod);
